...
function render_callback( $attributes )
{
  ...
  if ( empty($blocks) ) {
    ...
    if( $is_backend == true ) {
      if ($attributes['no_title'] == false) {
        $html = '<h2 class="simpletoc-title ' . $alignclass . '">' . __('Содержание', 'simpletoc') . '</h2>';
      }
  ...
  if ( empty( $headings_clean ) ) {
    ...
    if( $is_backend == true ) {
      ...
      if ($attributes['no_title'] == false) {
        $html = '<h2 class="simpletoc-title ' . $alignclass . '">' . __('Оглавление', 'simpletoc') . '</h2>';
      }
...
function generateToc( $headings, $attributes )
{
  ...
  if ($attributes['no_title'] == false) {
    $html = "<h2 class=\"simpletoc-title\">" . __("Оглавление", "simpletoc") . "</h2>";
  }
